...
#[AsCommand(
    name: 'app:db:import',
    description: 'Importa un dump nel database (Postgres)'
)]
class DatabaseImportCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addOption(
                ...
                'Pulisce gli oggetti esistenti prima del restore (pg_restore --clean --if-exists)'
            )
            ->addOption(
                'data-only',
                null,
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!$filesystem->exists($sourcePath)) {
            $io->error(sprintf('File di dump non trovato: %s', $this->relativePath($sourcePath)));
        ...
        $isSql = str_ends_with(strtolower($sourcePath), '.sql');
        $args = [];
        ...
        if (!$process->isSuccessful()) {
            $io->error('Errore durante l\'import del database.');
            $io->text($process->getErrorOutput() ?: $process->getOutput());
            return Command::FAILURE;
        }

        $io->success(sprintf('Import completato da %s', $this->relativePath($sourcePath)));
