...
- **Date timeline**: `{{DATE}}`, `{{START_DATE}}`, `{{END_DATE}}`, `{{PICKUP_DATE}}`, `{{DELIVERY_DATE}}`, `{{DISPATCH_DATE}}`, `{{ARRIVAL_DATE}}`, `{{DEPARTURE_DATE}}`, `{{SEIZURE_DATE}}`, `{{INCIDENT_DATE}}`, ecc. → riutilizzare `Income.signingDay/Year`, `paymentDay/Year`, `expirationDay/Year`, `cancelDay/Year`; altre date restano specifiche del template. Formato standard: `DDD/YYYY`.
- **Delivery proof**: `{{DELIVERY_PROOF_REF}}`, `{{DELIVERY_PROOF_DATE}}`, `{{DELIVERY_PROOF_RECEIVED_BY}}` → campi di consegna/accettazione specifici del dettaglio (freight, mail, trade, passengers, services, charter, subsidy).
...
- **Quantità/descrizioni operative**: `{{CARGO_DESCRIPTION}}`, `{{CARGO_QTY}}`, `{{TOTAL_MASS}}`, `{{PACKAGE_COUNT}}`, `{{GOODS_DESCRIPTION}}`, `{{QTY}}`, `{{GRADE}}`, `{{BATCH_IDS}}`, `{{PRIZE_DESCRIPTION}}`, `{{RECOVERED_ITEMS_SUMMARY}}`, `{{SERVICE_TYPE}}`, `{{WORK_SUMMARY}}`, `{{MAIL_TYPE}}`, `{{SECURITY_LEVEL}}`, `{{SEAL_CODES}}`, `{{BAGGAGE_ALLOWANCE}}`, `{{EXTRA_BAGGAGE}}`, `{{CLASS_OR_BERTH}}`, ecc. → dati operativi liberi (preferibilmente su `Income` o note).
  - Nota: nel dominio Traveller `dtons` indica “displacement tons”, unità volumetrica (~14 m³) usata per cargo e stive, non tonnellate di massa.
...
- **CHARTER**: CHARTER_ID, CHARTERER_NAME/CONTACT/SIGN, CARRIER_NAME/SIGN, VESSEL_NAME, DATE/START/END, AREA_OR_ROUTE, PURPOSE, MANIFEST_SUMMARY, DELIVERY_PROOF_REF/DATE/RECEIVED_BY, PAYMENT_TERMS, DEPOSIT, EXTRAS, DAMAGE_TERMS, CANCELLATION_TERMS, NOTES.
- **SUBSIDY**: SUBSIDY_ID, AUTHORITY_NAME/CONTACT/SIGN, CARRIER_NAME/CONTACT/SIGN, PROGRAM_REF, VESSEL_NAME, ORIGIN→DESTINATION, START/END, DELIVERY_PROOF_REF/DATE/RECEIVED_BY, SERVICE_LEVEL, SUBSIDY_AMOUNT, PAYMENT_TERMS, MILESTONES, REPORTING_REQUIREMENTS, NON_COMPLIANCE_TERMS, CANCELLATION_TERMS, PROOF_REQUIREMENTS, NOTES.
...
- **FREIGHT**: CONTRACT_ID, SHIPPER_NAME/CONTACT/SIGN, CARRIER_NAME/SIGN, VESSEL_NAME, ORIGIN→DESTINATION, PICKUP/DELIVERY, DELIVERY_PROOF_REF/DATE/RECEIVED_BY, CARGO_DESCRIPTION/QTY/DECLARED_VALUE, PAYMENT_TERMS, LIABILITY_LIMIT, CANCELLATION_TERMS, NOTES.
- **SERVICES**: SERVICE_ID, CUSTOMER_NAME/CONTACT/SIGN, PROVIDER_NAME/SIGN, LOCATION, VESSEL_NAME/ID, SERVICE_TYPE, REQUESTED_BY, START/END, DELIVERY_PROOF_REF/DATE/RECEIVED_BY, WORK_SUMMARY, PARTS_MATERIALS, RISKS, PAYMENT_TERMS, EXTRAS, LIABILITY_LIMIT, CANCELLATION_TERMS, NOTES.
- **PASSENGERS**: TICKET_ID, PASSENGER_NAMES/CONTACT/SIGN, CARRIER_NAME/SIGN, VESSEL_NAME, ORIGIN→DESTINATION, DEPARTURE/ARRIVAL, DELIVERY_PROOF_REF/DATE/RECEIVED_BY, CLASS_OR_BERTH, QTY, BAGGAGE_ALLOWANCE/EXTRA_BAGGAGE, PAYMENT_TERMS, REFUND_CHANGE_POLICY, NOTES.
...
- **MAIL**: CARRIER_NAME/SIGN, VESSEL_NAME, ORIGIN→DESTINATION, DISPATCH/DELIVERY, DELIVERY_PROOF_REF/DATE/RECEIVED_BY, MAIL_TYPE, PACKAGE_COUNT, TOTAL_MASS, SECURITY_LEVEL, SEAL_CODES, PAYMENT_TERMS, PROOF_OF_DELIVERY, LIABILITY_LIMIT, NOTES.
- **INSURANCE**: CLAIM_ID, INSURER_NAME/SIGN, INSURED_NAME/CONTACT/SIGN, POLICY_NUMBER (Income.code), VESSEL_NAME, INCIDENT_REF/DATE/LOCATION/CAUSE/LOSS_TYPE, VERIFIED_LOSS, DEDUCTIBLE, COVERAGE_NOTES, PAYMENT_TERMS, ACCEPTANCE_EFFECT, SUBROGATION_TERMS, NOTES.
...
- **TRADE**: DEAL_ID, BUYER_NAME/CONTACT/SIGN, SELLER_NAME/CONTACT/SIGN, LOCATION, GOODS_DESCRIPTION, QTY, GRADE, BATCH_IDS, UNIT_PRICE, PAYMENT_TERMS, DELIVERY_METHOD/DATE, DELIVERY_PROOF_REF/DATE/RECEIVED_BY, TRANSFER_POINT/CONDITION, AS_IS_OR_WARRANTY/WARRANTY, CLAIM_WINDOW, CANCEL_RETURN_POLICY, NOTES.
